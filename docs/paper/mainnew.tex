...
The remainder of the paper summarizes related work (Section~II), presents the system architecture (Section~III), details the methodology including the Federated Learning--RAG bridge (Sections~IV--V), reports experimental results (Section~VI), and concludes with limitations and future work (Sections~VII--VIII).
...
\begin{tikzpicture}[
  ...
  bigbox/.style={box, minimum width=0.25\textwidth},
  arr/.style={-Latex, thick}
]

% --- Inference Nodes ---
\node[bigbox] (a1) {Agent 1\\Federated Autoencoder\\Anomaly gate $s(x)$};
% Increased horizontal distance to 2.2cm to clear text
\node[bigbox, right=2.2cm of a1] (a2) {Agent 2\\XGBoost};
\node[bigbox, right=2.2cm of a2] (a3) {Agent 3\\RAG + LLM\\Grounded actions};
\node[box, right=1cm of a3, minimum width=0.18\textwidth] (mit) {Mitigation\\commands / tickets};

\node[box, above=1cm of a3, minimum width=0.22\textwidth] (kb) {Threat KB\\ATT\&CK + CVE + IR guides};
\node[box, below=1.2cm of a3, minimum width=0.22\textwidth] (bridge) {Fed--RAG bridge\\drift/significance\\trigger re-analysis};

% --- Internal Connections ---
% Added "above=1mm" to ensure labels do not touch the arrows
\draw[arr] (a1) -- node[above=1mm] {flagged flows} (a2);
\draw[arr] (a2) -- node[above=1mm] {Unknown} (a3);
\draw[arr] (a3) -- (mit);
% Added "right=1mm" to push text away from vertical arrows
\draw[arr] (kb) -- node[right=1mm, align=left] {retrieve\\evidence} (a3);
\draw[arr] (bridge.north) -- node[right=1mm, align=left] {refresh queries\\and plans} (a3.south);

% --- External Input ---
% Extended the coordinate further left and lifted the text
\coordinate (bridge_in) at ([xshift=-4cm]bridge.west);
% --- Bounding Box ---
\node[fit=(a1)(a2)(a3)(kb)(bridge)(mit)(bridge_in), draw, rounded corners, inner sep=5pt, label={[font=\small]}] {};

\end{tikzpicture}%
}
\caption{Overall agentic pipeline architecture.}
...
\centering
\begin{tikzpicture}[
    % Overall scale for double-column fit
    scale=0.85, transform shape,
    node distance=1.5cm and 2cm,
    % Professional Box Styles
    stepbox/.style={rectangle, draw=black, thick, rounded corners, minimum width=3.8cm, minimum height=1.1cm, align=center, fill=blue!5, font=\sffamily\normalsize},
    agentbox/.style={rectangle, draw=none, fill=blue!70, text=white, font=\sffamily\bfseries\normalsize, minimum width=4cm, minimum height=2.8cm, align=center},
    resultbox/.style={rectangle, draw=black, thick, rounded corners, minimum width=3.8cm, minimum height=1.1cm, align=center, fill=gray!10, font=\sffamily\normalsize},
    % Frame Style
    mainframe/.style={rectangle, draw=black, thick, rounded corners=0.8cm, fill=white},
    arrow/.style={->, >=Stealth, thick, line width=0.9pt}
]
    ...
    % --- 2. COLUMN 1: Initial Processing ---
    \node (traffic) [stepbox] {Network Traffic};
    \node (clean) [stepbox, below=of traffic] {Data Cleaning};
    \node (split) [stepbox, below=of clean] {Data Splitting for\\zero shot setup};

    % --- 3. COLUMN 2: Feature Engineering ---
    % CHANGED HERE: Positioned relative to 'split' instead of 'traffic'
    \node (scale) [stepbox, right=2.5cm of split] {Feature Scaling};
    \node (balance) [stepbox, above=of scale] {Data Balancing};
    \node (resample) [stepbox, above=of balance] {Resampled Data Splitting};

    % --- 4. COLUMN 3: Analysis & Results ---
    \node (agents) [agentbox, right=3cm of resample] {SECURITY AGENTS};
    \node (decision) [resultbox, below=1.5cm of agents] {Test Decisions for\\Threat Mitigation};
    \node (results) [resultbox, below=of decision] {Security Results};

    % --- 5. FRAME (Background Layer) ---
    \begin{pgfonlayer}{background}
        \node (frame) [mainframe, fit=(traffic) (split) (resample) (results) (agents), inner sep=1cm] {};
    \end{pgfonlayer}

    % --- 6. DRAWING ARROWS ---
    
    % Column 1 Vertical
    \draw[arrow] (traffic) -- (clean);
    \draw[arrow] (clean) -- (split);

    % Link Col 1 to Col 2
    \draw[arrow] (split.east) -- (scale.west);

    % Column 2 Vertical
    \draw[arrow] (scale) -- (balance);
    \draw[arrow] (balance) -- (resample);

    % Link Col 2 to Agents
    \draw[arrow] (resample.east) -- (agents.west);

    % Column 3 Vertical (Flowing Upwards)
    \draw[arrow] (agents.south) -- (decision);
    \draw[arrow] (decision) -- (results);

\end{tikzpicture}
\caption{Comprehensive Methodology Flowchart and Data Processing Pipeline}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{UNSWNB15_data_imbalance.png}
    \caption{UNSW-NB15 Attack class distribution.}
    \label{fig:my_image}
\end{figure}
...
\textbf{Preprocessing.} For Agents~1 and~2, features are transformed using the shared pipeline configuration (fit on the training split and reused unchanged at test time). Numerical features are min--max scaled by default; categorical fields (\texttt{proto}, \texttt{service}, \texttt{state}) are label-encoded; and binary indicators are retained as 0/1. This yields $d=42$ input features (37 numerical + 3 categorical + 2 binary) after preprocessing.

The anomaly score produced by Agent~1 can be appended to the Agent~2 feature vector as an auxiliary confidence signal, providing a consistent measure of ``distance from normality''.
...
Agent~1 is a feed-forward PyTorch autoencoder trained to reconstruct benign-dominant traffic. The architecture follows a symmetric bottleneck design (input dimension $d=42$ after preprocessing):
\begin{equation}
42 \rightarrow 32 \rightarrow 16 \rightarrow 8 \rightarrow 16 \rightarrow 32 \rightarrow 42,
\end{equation}
...
\begin{equation}
\begin{split}
q = \{ \hat{y},\, c(x),\, s(x),\, \text{feature importance/profile},\, \text{zero-day flag}, \\
\text{historical scope} \}.
\end{split}
\end{equation}
...
\begin{tikzpicture}[
    ...
    % Box styles
    outerbox/.style={rectangle, draw=black, thick, minimum width=4.5cm, minimum height=1.5cm, align=center, font=\sffamily\bfseries\normalsize, fill=white},
    vectordb/.style={rectangle, draw=none, fill=cyan!70!blue, text=white, minimum width=5cm, minimum height=1.2cm, align=center, font=\sffamily\bfseries\normalsize},
    knowledgebase/.style={rectangle, draw=none, fill=cyan!10, text=black, minimum width=5cm, minimum height=1.2cm, align=center, font=\sffamily\bfseries\normalsize},
    rightbox/.style={rectangle, draw=black, rounded corners, minimum width=5.5cm, align=left, fill=gray!5, text=black, inner sep=10pt, font=\sffamily\normalsize},
    % Frame style
    mainframe/.style={rectangle, draw=black, thick, rounded corners=0.8cm, fill=white},
    ...
    % --- 1. DEFINE LAYERS ---
    % This explicitly tells TikZ that 'background' is behind 'main'
    \pgfdeclarelayer{background}
    \pgfsetlayers{background,main}

    % --- 2. DRAW INTERNAL NODES (Main Layer) ---
    \node (vector) [vectordb] {Vector DataBase \\ FAISS + LangChain};
    \node (knowledge) [knowledgebase, below=4.5cm of vector] {Knowledge Base \\ MITRE ATT\&CK + CVE/NVD};

    \node (retriever) [rightbox, right=of vector.east, anchor=west] {
        \textbf{Retriever} \\ FAISS Vector Database
    };
    \node (augmenter) [rightbox, below=1cm of retriever] {
        \textbf{Augmenter} \\
        \hspace{0.2cm}$\bullet$ Similarity score \\
        \hspace{0.2cm}$\bullet$ Threat type \\
        \hspace{0.2cm}$\bullet$ Retrieval info
    };
    \node (llm) [rightbox, below=1cm of augmenter] {
        \textbf{LLM Reasoning Engine} \\ Groq Llama 3
    };
    \node (explained) [rightbox, below=1cm of llm] {
        \textbf{Explained Threat} \\
        \hspace{0.2cm}$\bullet$ Why this is novel \\
        \hspace{0.2cm}$\bullet$ Possible attack type
    };

    % --- 3. DRAW FRAME ON BACKGROUND LAYER ---
    \begin{pgfonlayer}{background}
        \node (frame) [mainframe, fit=(vector) (knowledge) (retriever) (explained), inner sep=0.8cm] {};
    \end{pgfonlayer}

    % --- 4. POSITION AGENT TWO ---
    \node (agenttwo) [outerbox, left=3cm of frame.west] {Agent Two: \\ XGBoost Classifier};

    % --- 5. ARROWS ---
    \draw[arrow] (agenttwo.east) -- (frame.west); 

    \draw[arrow] (knowledge.north) -- (vector.south) 
        node[midway, fill=white, inner sep=4pt, align=center, font=\sffamily\large] {Vector Embedding \\ HuggingFace};

    \draw[arrow] (vector.east) -- (retriever.west);
    \draw[arrow] (retriever.south) -- (augmenter.north);
    \draw[arrow] (augmenter.south) -- (llm.north);
    \draw[arrow] (llm.south) -- (explained.north);

\end{tikzpicture}
\caption{Architectural Detail of the RAG Explanation Pipeline}
\label{fig:rag_pipeline_final}
\end{figure*}
...
\begin{table*}[t]
\caption{Action-space mapping used for deterministic fallbacks and evaluation. Agent~2 produces fine-grained UNSW-NB15 categories; for response generation, they are mapped to a smaller set of operator-oriented action categories.}
...
\toprule
\textbf{ID} & \textbf{Category} & \textbf{UNSW-NB15 attacks} & \textbf{CIC-IDS2017 attacks} & \textbf{MITRE techniques}\\
\midrule
0 & Normal & Normal & BENIGN & --\\
1 & DoS/DDoS & DoS & DoS Hulk; DDoS; Slowloris & T1498, T1499\\
2 & Reconnaissance & Reconnaissance & PortScan & T1046, T1595\\
3 & Exploits & Exploits; Shellcode & Web Attack-XSS; SQL Injection & T1190, T1059\\
4 & Brute\_Force & Fuzzers & FTP-Patator; SSH-Patator & T1110, T1078\\
5 & Malware & Backdoor; Worms; Generic & Bot; Infiltration & T1059, T1071\\
6 & Analysis & Analysis & Heartbleed & T1040, T1557\\
\bottomrule
\end{tabularx}
\end{table*}


\subsection{Federated Learning--RAG Bridge Methodology}
...
\subsubsubsection{Significance levels}
...
\subsubsubsection{Retrieval step}
...
\subsubsubsection{Generation step}
...
\textbf{Dataset and labels.} UNSW-NB15 attack categories (e.g., \emph{DoS}, \emph{Exploits}, \emph{Reconnaissance}, \emph{Backdoor}, \emph{Fuzzers}, \emph{Shellcode}, \emph{Worms}) plus \emph{Normal} traffic are used \cite{Moustafa2015UNSW}. Agent~1 is evaluated as a binary gate (Normal vs Attack), while Agent~2 is evaluated as a multi-class classifier on known categories.

\textbf{Zero-day definition: LOAO.} ``Zero-day'' is defined using a \emph{Left-One-Attack-Out (LOAO)} test. For each organization/client, training is performed on a filtered local dataset that excludes one attack family; the excluded family becomes that client’s ``never-seen'' (zero-day) pattern at test time.

\textbf{LOAO configuration.} Three clients are used:
\begin{itemize}
...
\textbf{Federated setup (Agent~1 only).} $R=10$ federated rounds are run with FedAvg \cite{mcmahan2017fedavg}, with three clients participating each round and one local epoch per round. The per-round \emph{global} validation accuracy is reported at the server (computed after aggregation), and a generally improving trend with non-monotonic fluctuations is observed under non-IID client data.
...
Agent~1 separates benign and malicious flows using reconstruction error. The operating threshold is selected on a held-out validation split (targeting a low false-positive regime), and the decision threshold is set to $\tau=0.0712$, yielding high attack recall while maintaining low per-flow latency on CPU.

\begin{table*}[t]
\caption{Agent~1 (Autoencoder) gate performance on UNSW-NB15.}
...
Very high attack recall (\(\approx\)99\%) is achieved by the autoencoder gate while remaining fast enough for line-rate filtering on CPU. As a result, heavier components (e.g., LLM) can be avoided for the majority of benign flows.
...
Agent~2 classifies traffic into known categories using XGBoost \cite{Chen2016XGBoost}. To reduce class imbalance effects, SMOTE oversampling is applied together with class weights and per-class decision thresholds \cite{Chawla2002SMOTE}.
...
\midrule
Accuracy & 0.832 & Known-class test set\\
Macro-F1 & 0.609 & Robust to class imbalance\\
Weighted-F1 & 0.817 & Dominated by frequent classes\\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Worms & 0.50 & Very low support; most variable\\
Shellcode & 0.73 & Minority class retained with usable recall\\
Backdoor & 0.74 & Strong recall under imbalance\\
\bottomrule
\end{tabular}
\end{table}

\begin{table*}[t]
\caption{Per-class F1-score (\%) for Agent~2 (XGBoost) on known UNSW-NB15 categories (held-out test set).}
...
\toprule
\textbf{Metric} & \textbf{Normal} & \textbf{DoS} & \textbf{Exploits} & \textbf{Recon.} & \textbf{Fuzzers} & \textbf{Backdoor} & \textbf{Analysis} & \textbf{Generic} & \textbf{Shellcode} & \textbf{Worms}\\
\midrule
F1 (\%) & 20.5 & 76.4 & 99.0 & 82.0 & 20.7 & 74.0 & 19.8 & 93.5 & 73.3 & 50.0\\
\bottomrule
\end{tabular}}
\end{table*}
...
In LOAO, each client’s held-out attack family is treated as unknown at test time. Unknownness is scored using model uncertainty (e.g., $1-\max_c p(c\mid x)$), and ``Unknown/Zero-day'' is declared when uncertainty exceeds a fixed threshold.
...
Table~\ref{tab:fl_sim} and Fig.~\ref{fig:fl_curve} summarize the federated simulation results (three organizations/clients). We report the \emph{global} validation accuracy after each FedAvg aggregation step. Under non-IID data, the trajectory shows an overall improving trend with occasional drops.
Table~\ref{tab:fl_sim} and Fig.~\ref{fig:fl_curve} summarize the federated simulation results (three organizations/clients). The \emph{global} validation accuracy is reported after each FedAvg aggregation step. Under non-IID data, an overall improving trend is seen, with occasional drops.

\begin{table}[H]
\caption{Federated learning simulation results (global validation accuracy after FedAvg, \%).}
\label{tab:fl_sim}
\centering
\begin{tabular}{cc}
\toprule
\textbf{Round} & \textbf{Global accuracy (\%)}\\
\midrule
1  & 68.48\\
2  & 70.05\\
3  & 73.91\\
...
\begin{axis}[
  ...
  ylabel={Global validation accuracy (\%)},
  xmin=1,
  xmax=10,
  ymin=60,
  ymax=90,
...
\addplot+[mark=*, thick] coordinates {
  (1,68.48) (2,70.05) (3,73.91) (4,77.50) (5,79.30)
  (6,82.68) (7,81.12) (8,85.82) (9,85.27) (10,86.63)
};
\addlegendentry{Global}
\end{axis}
\end{tikzpicture}
\caption{Federated-round trajectory (global validation accuracy after FedAvg). The non-monotonicity is consistent with non-IID client updates.}
\label{fig:fl_curve}
\end{figure}
...
LOAO results indicate that the pipeline generalizes to previously unseen attacks at each client. As federated rounds progress, anomaly separation is improved by Agent~1 on each client’s held-out family, increasing the probability that zero-day samples are escalated to Agent~3 for grounded response guidance. In parallel, low-confidence samples from held-out families are captured by Agent~2’s ``Unknown'' mode (Table~\ref{tab:unknown_main}).

\subsection{Real-Traffic Streaming Evaluation}
\label{sec:real_traffic}

To validate operational feasibility beyond offline benchmarks, the pipeline was tested in an \emph{online streaming replay} setting by replaying raw flow records through the flow-ingestion component (\texttt{pkt-streamer}). Concretely, the raw UNSW-NB15 trace split file (\texttt{UNSW-NB15\_1.csv}, $n=700{,}001$ flows) is streamed as a traffic-like sequence of flow events to the local detection API (\texttt{/detect}), approximating a deployment where flows arrive continuously.
...
\begin{table}[t]
\caption{Real-traffic streaming results (operational metrics).}
...
\begin{axis}[
  ...
  height=0.55\columnwidth,
  ylabel={Latency (ms)},
  symbolic x coords={Agent1,Agent2,FastPath,Escalation},
  xtick=data,
  xticklabels={Agent~1,Agent~2,Agent~1+2 (p95),Escalation (p95)},
  grid=both,
]
\addplot coordinates {(Agent1,0.8) (Agent2,0.6) (FastPath,3.4) (Escalation,690)};
\end{axis}
\end{tikzpicture}
...
\subsection{FL $\rightarrow$ RAG Bridge Results}
...
We evaluate the bridge’s analyst-facing outputs with a lightweight, auditable 0--1 rubric intended as a \emph{proxy} for actionability and grounding (full human-in-the-loop validation remains future work).

\textbf{Bridge evaluation protocol (logged run).} For each federated round $t\in\{1,\dots,10\}$, the bridge logs (i) the number of trigger events and (ii) the number of queued samples reprocessed (Fig.~\ref{fig:trigger_dynamics}). The totals reported here (64 trigger events and 712 reprocessed samples) correspond to the \emph{specific end-to-end run used to generate Figs.~\ref{fig:explanation_quality}--\ref{fig:trigger_dynamics}}; these counts vary with drift thresholds, client non-IIDness, and traffic mix.

For each reprocessed sample at round $t$, Agent~3 emits a structured explanation (threat summary, ``why novel'' note if applicable, and a prioritized \textbf{Recommended Actions} list) together with the retrieved evidence identifiers (e.g., ATT\&CK technique IDs and CVE IDs). We compute a per-sample rubric score and then report the per-round mean across reprocessed samples.

\textbf{Rubric definition (0--1, deterministic proxy).} Let $E(x,t)$ denote the generated explanation for sample $x$ at round $t$, and let $R(x,t)$ denote the set of retrieved passages/records provided to the generator. The aggregate score is the mean of three sub-scores in $[0,1]$:
\begin{itemize}
  \item \emph{Grounding consistency} $g(E,R)$: parse $E$ for explicit identifiers (ATT\&CK technique IDs and CVE IDs) and score 1 if every referenced identifier also appears in $R$; otherwise 0.
  \item \emph{Action completeness} $a(E)$: score 1 if $E$ contains at least three concrete, ordered actions (containment/mitigation/verification) and at least two map to the discrete action space in Table~\ref{tab:action_space}; otherwise 0.
  \item \emph{Specificity} $s(E)$: score 1 if $E$ includes at least one concrete indicator or operational constraint (e.g., protocol/service/state cue, threshold/score, or named control such as blocking, rate limiting, isolation); otherwise 0.
\end{itemize}
This proxy score is intentionally conservative: it does not reward fluent text unless it is grounded, actionable, and specific.

\textbf{Improved explanation evaluation (recommended).} To further reduce the risk of over-claiming, future evaluations should include (i) a SOC-style human study (time-to-triage, mitigation correctness, trust calibration) and (ii) automated faithfulness metrics such as grounding precision/citation coverage and action-space consistency.
...
\end{axis}
\end{tikzpicture}
\caption{Explanation quality over rounds (aggregate 0--1 rubric score; representative logged run). Points show the per-round mean over reprocessed samples; monotonic improvement is not guaranteed across runs.}
...
\end{axis}
\end{tikzpicture}
\caption{FL+RAG bridge trigger dynamics across rounds (representative logged run). Counts are extracted from bridge/coordinator logs and depend on drift thresholds and client/traffic heterogeneity.}
...
Table~\ref{tab:ablations} summarizes the three requested baseline axes using results already reported in this paper. (i) \textbf{No RAG vs RAG:} adding RAG/LLM escalation leaves detection metrics unchanged but increases tail latency, reflecting that Agent~3 is invoked selectively for action planning rather than improving classifier accuracy. (ii) \textbf{No FL vs FL:} the non-federated and federated full-pipeline variants achieve comparable offline detection quality, while FL provides governance benefits (no raw data sharing) and robustness to non-IID client environments as shown by the round-wise convergence in Table~\ref{tab:fl_sim}. (iii) \textbf{No bridge vs bridge:} Table~\ref{tab:flrag_vs_trad} and Figs.~\ref{fig:explanation_quality}--\ref{fig:trigger_dynamics} capture the operational value of the bridge (refresh + reprocessing) beyond using FL or RAG in isolation.

\begin{table*}[t]
\caption{Ablation study and baseline comparisons (reported metrics).}
\label{tab:ablations}
\centering
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}} l c c c @{}}
\toprule
...
Non-federated Agent~1 + RAG/LLM (full pipeline) & 0.98 & 0.89 & 680\\
